...
- `CurveOptions` gains an optional `type` (an `ActivityType`): the API requires it for power curves — `getPowerCurves()` returns HTTP 422 without it — and accepts it for pace and HR curves.
