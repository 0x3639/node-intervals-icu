...
### Added
- `IntervalsAPIError.details` carries the response body the API sent with an error, and the body's `error` (or `message`) text is appended to `message`, so a `422` explains itself (e.g. `Cannot send message to self`).

### Security
- Every GitHub Actions workflow pins its actions to a full commit SHA (the major tag is kept as a comment).

### Fixed
- The chat send/edit/delete example and live test sent the message to the caller, which the API rejects (`422 Cannot send message to self`); both now take a consenting recipient (`INTERVALS_CHAT_TO_ATHLETE_ID` / `INTERVALS_LIVE_CHAT_TO`).
