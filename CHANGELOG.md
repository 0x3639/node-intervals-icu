...
- `WorkoutConversionInput` and `WorkoutDoc` (exported from the package entrypoint): the body type for `convertWorkout()` / `convertWorkoutForAthlete()`; `name`, `description`, `type` and a non-null object `workout_doc` are required because the live API returns HTTP 500 without them. `Event.workout_doc` is now typed `WorkoutDoc` instead of `any`.
