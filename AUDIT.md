...
By default the probe sends only GET requests. A probe whose SDK or spec form uses POST/PUT/DELETE is skipped unless `INTERVALS_LIVE_WRITE=1` is set; even then, it sends a malformed body where the route takes a body, or uses a sentinel id that cannot correspond to real data (wellness date 1900-01-01, shared-event id 0) for bodyless DELETEs. This is best-effort safety, not proof that a handler cannot mutate data — run it against an account whose data you can afford to risk.
