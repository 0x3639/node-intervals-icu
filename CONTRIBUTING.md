...
- **Report Bugs**: Found a bug? Please open an issue with detailed information about the problem
- **Suggest Features**: Have an idea for a new feature? Open an issue to discuss it
- **Improve Documentation**: Help make the docs clearer and more comprehensive
- **Write Code**: Submit pull requests to fix bugs or add new features
- **Share Feedback**: Let us know how you're using the library and what could be improved
...
2. Make sure it aligns with the Intervals.icu API capabilities
...
- `npm run coverage:api` — diff the SDK's HTTP calls against the vendored spec. The baseline (`spec/coverage-baseline.json`) currently has 0 phantom ops; 3 verified-but-undocumented routes are allowlisted separately in `spec/undocumented-routes.json` (see [AUDIT.md](./AUDIT.md)) and excluded from the phantom count. The gate fails on any regression: a new SDK operation not in the spec, not in the baseline, and not in the allowlist (a new phantom op), a previously-covered spec operation that is no longer matched (lost coverage), or a baseline/allowlist entry that is stale (a phantom op that's no longer phantom, a newly-covered spec op not yet recorded, or an allowlist entry that's no longer phantom — any of these must be updated so the baseline/allowlist stay accurate). Use `npm run coverage:api -- --write-baseline` (or `node scripts/coverage.mjs --write-baseline`) to regenerate the baseline after resolving an entry, and `node scripts/coverage.mjs --strict` to ignore the baseline and fail on any phantom or missing op (the eventual 149/149 gate)
...
- Include examples in the PR description when applicable
...
**Future goal**: We welcome contributions to add a comprehensive test suite!
...
Feel free to open an issue with your question, or reach out to the maintainer. We're here to help!
